...
*   **Setting:** Over the ocean with scrolling water and occasional islands.
*   **Graphics:** Use Canvas API to draw clean vector-style sprites (no external images).
    *   **Player:** A sleek WWII-style fighter plane (green/grey).
    *   **Enemies:** Red/Dark planes, different sizes for small fighters and large bombers.
    *   **Bullets:** Bright yellow for player, orange/red orbs for enemies.
    *   **Explosions:** Particle effects when enemies are destroyed.

### 2. Gameplay
*   **Movement:** 
    *   Desktop: Arrow keys or WASD to move the plane.
    *   Mobile: Touch and drag to move the plane directly.
*   **Combat:**
    *   Auto-fire or Spacebar to shoot.
    *   Enemies fly down in formations. Some shoot back.
*   **Systems:**
    *   Score tracking.
    *   HP system (Player can take multiple hits).
    *   Game Over screen with Restart button.

### 3. Technical
*   **Single File:** All HTML, CSS, and JavaScript in one index.html file.
*   **Responsiveness:** Canvas fits the screen (maintain aspect ratio suitable for vertical shooter).
...
window.addEventListener('message', (e) => {
  if (e.data && e.data.type === 'PAUSE_GAME') {
    // Logic to set a global isPaused variable or similar
            ...
            <div style={{...styles.modalSub, fontSize: '15px', color: '#ccc'}}>
              ...
              <div style={{background: 'rgba(255,255,255,0.05)', padding: '16px', borderRadius: '12px', marginTop: '20px'}}>
                <p style={{margin: '0 0 12px 0', color: '#fff'}}>
                  <strong>Experiment:</strong> This game was <strong>one-shot generated</strong> by Google's Gemini 3 Pro.
                </p>
      ...
      {showRemix && (
        <div style={styles.modalOverlay} onClick={() => setShowRemix(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            {['Double Speed', 'Hard Mode', 'Infinite Ammo'].map((item, i) => (
               <div 
                key={item}
              ...
              >
                ...
                <div>
                  ...
                  <div style={{fontSize: '13px', color: '#888'}}>
                    {['Run game logic at 200% speed', 'More enemies, faster bullets', 'Never reload / Rapid fire'][i]}
                  </div>
                </div>
              </div>
            ))}
